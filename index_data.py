...
def find_new_rows_to_index(df):
    ...
    if df.empty or 'call_id' not in df.columns:
        print("WARNING: DataFrame is empty or missing 'call_id' column")
        return pd.DataFrame()
    ...
    try:
        ...
        for i in range(0, len(all_sheet_ids), 100):
            ...
            fetch_response = index.fetch(ids=batch_ids, namespace="")  # FIXED: Explicitly specify namespace
            existing_ids.update(fetch_response.vectors.keys())
            print(f"Checked batch {i//100 + 1}/{(len(all_sheet_ids)-1)//100 + 1}")
        ...
        print(f"ERROR in find_new_rows_to_index: {e}")
        import traceback
        traceback.print_exc()
        return pd.DataFrame()
...
def compute_daily_aggregates(df):
    ...
    try:
        ...
        print(f"ERROR in compute_daily_aggregates: {e}")
        import traceback
        traceback.print_exc()
        return []
...
def compute_overall_aggregates(df):
    ...
    try:
        ...
        print(f"ERROR in compute_overall_aggregates: {e}")
        import traceback
        traceback.print_exc()
        return {}
...
Total calls: {overall_agg.get('total_calls', 0)}
Date range: {overall_agg.get('date_range', {}).get('start', 'N/A')} to {overall_agg.get('date_range', {}).get('end', 'N/A')}
Sentiment distribution: {', '.join([f"{k}: {v}" for k, v in overall_agg.get('sentiment_distribution', {}).items()])}
Top issues: {', '.join([f"{k} ({v} calls)" for k, v in list(overall_agg.get('top_10_issues', {}).items())[:5]])}
Average satisfaction: {overall_agg.get('avg_satisfaction', 0):.2f}/5
Resolution status: {', '.join([f"{k}: {v}" for k, v in overall_agg.get('resolution_breakdown', {}).items()])}
Average call duration: {overall_agg.get('avg_duration', 0):.0f} seconds"""

        print("Creating overall aggregate embedding...")
        ...
        vectors.append({
            ...
            "metadata": {
                ...
                "total_calls": overall_agg.get('total_calls', 0),
                "date_range_start": overall_agg.get('date_range', {}).get('start', 'N/A'),
                "date_range_end": overall_agg.get('date_range', {}).get('end', 'N/A'),
                "avg_satisfaction": overall_agg.get('avg_satisfaction', 0),
                "generated_at": datetime.now().isoformat()
            }
        })
        
        # Daily aggregate vectors (last 30 days)
        daily_aggs_to_process = daily_aggs[-30:] if len(daily_aggs) > 30 else daily_aggs
        print(f"Creating {len(daily_aggs_to_process)} daily aggregate embeddings...")
        
        for idx, daily_agg in enumerate(daily_aggs_to_process, 1):
            daily_text = f"""Daily Statistics for {daily_agg.get('date', 'N/A')}:
Total calls: {daily_agg.get('total_calls', 0)}
Sentiments: {', '.join([f"{k}: {v}" for k, v in daily_agg.get('sentiment_counts', {}).items()])}
Top issues: {', '.join([f"{k} ({v})" for k, v in list(daily_agg.get('top_issues', {}).items())[:3]])}
Average satisfaction: {daily_agg.get('avg_satisfaction', 0):.2f}/5
Urgency: {', '.join([f"{k}: {v}" for k, v in daily_agg.get('urgency_distribution', {}).items()])}"""
            ...
            vectors.append({
                "id": f"aggregate_daily_{daily_agg.get('date', 'unknown')}",
                "values": response.data[0].embedding,
                "metadata": {
                    "type": "aggregate_daily",
                    "date": daily_agg.get('date', 'N/A'),
                    "summary": daily_text,
                    "total_calls": daily_agg.get('total_calls', 0),
                    "avg_satisfaction": daily_agg.get('avg_satisfaction', 0)
                }
            })
            
            if idx % 5 == 0:
                print(f"  Processed {idx}/{len(daily_aggs_to_process)} daily aggregates")
        
        print(f"Created {len(vectors)} aggregate vectors successfully")
        return vectors
    
    except Exception as e:
        print(f"ERROR in create_aggregate_vectors: {e}")
        import traceback
        traceback.print_exc()
        return []
...
def create_embeddings_and_upsert(df):
    ...
    try:
        for i in range(0, len(df), batch_size):
            ...
            # FIXED: Explicitly specify namespace
        ...
        print(f"ERROR in create_embeddings_and_upsert: {e}")
        import traceback
        traceback.print_exc()
...
def upsert_aggregate_vectors(vectors):
    ...
    if not vectors:
        print("No aggregate vectors to upsert")
        return
    ...
    try:
        ...
        try:
            ...
            print(f"Warning: Could not clear old aggregates: {e}")
        ...
        print(f"ERROR in upsert_aggregate_vectors: {e}")
        import traceback
        traceback.print_exc()
